...
export default function TaskItem({ task, onToggle, onEdit, onDelete, isDeleting = false }: TaskItemProps) {
  return (
    <div className={clsx("flex items-center justify-between rounded-lg border p-4 shadow-sm transition-colors", task.status === 'DONE' ? 'bg-gray-50 border-gray-200' : 'bg-white border-gray-200 hover:border-black')}>
      <div className="flex items-center space-x-4">
        <button onClick={() => onToggle(task.id)} className="text-gray-500 hover:text-black">
          {task.status === 'DONE' ? (
      ...
      <div className="flex space-x-2">
        <button onClick={() => onEdit(task)} className="p-2 text-gray-500 hover:text-black">
